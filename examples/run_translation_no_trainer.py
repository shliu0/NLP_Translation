...
def parse_args():
    ...
    parser.add_argument("--tboard_dir", type=str, default=None, help="Where to store the tensorboard log file.")
...
def main():
    ...
    # in the environment
    accelerator = (
        Accelerator(log_with=args.report_to, project_dir=args.tboard_dir) if args.with_tracking else Accelerator()
    )
    ...
    if args.config_name:
        config = MBartConfig.from_pretrained(args.config_name, trust_remote_code=args.trust_remote_code)
    elif args.model_name_or_path:
        config = MBartConfig.from_pretrained(args.model_name_or_path, trust_remote_code=args.trust_remote_code)
    else:
    ...
    if args.model_name_or_path:
        ...
        model = MBartForConditionalGeneration.from_config(config, trust_remote_code=args.trust_remote_code)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.with_tracking:
            accelerator.log(
                {
                    f"{target_lang}-bleu": eval_metric["score"],
                    f"{target_lang}-train_loss": total_loss.item() / len(train_dataloader),
                    "epoch": epoch,
